...
from camera_handler import CameraHandler # DetectionEvent not explicitly used for type hint here
# import asyncio # No longer needed here
import logging
import time
from datetime import datetime, timedelta # timedelta still used for status log
# Simulation specific imports - should be commented out for production
...
def format_telegram_message(vip_details, detection_event, direction="N/A"):
    ...
    if direction_text == "IN" or "IN" in direction_text : # Handle "IN", "R-IN", "V/R-IN"
    ...
    separator = "------------------------"
    # No backticks for variables here, as escape_markdown_v2 in notifier handles it
...
def run_main_loop(config, cam_handler, vip_manager, telegram_notifier, camera_direction_map):
    ...
    try:
        while True:
            ...
            # if simulation_active and not simulation_triggered and (time.time() - simulation_timer_start > 5):
            #     logging.info("--- !!! TRIGGERING SIMULATED DETECTION EVENT (main.py) !!! ---")
            #     try:
            #         from PIL import Image, ImageDraw
            #         from io import BytesIO
            #         from camera_handler import DetectionEvent # Ensure DetectionEvent is available
            ...
            #         simulated_event = DetectionEvent(
            ...
            #         simulation_triggered = True # Ensure it only triggers once per app run if simulation_active is True
            #     except Exception as e_sim:
            #         logging.error(f"Error creating simulated event: {e_sim}", exc_info=True)
            #         detections = [] # Fallback to empty list
            # else:
            #     if not detections: # Only fetch if simulation didn't populate detections
            #         detections = cam_handler.get_new_detections(max_items=5, timeout=detection_fetch_interval)
            # --- END TEMPORARY SIMULATION CODE ---

            detections = cam_handler.get_new_detections(max_items=5, timeout=detection_fetch_interval) # Standard operation
                ...
                for det_event in detections:
                    ...
                    if vip_details:
                        ...
                        if chat_id_to_notify:
                            logging.debug(f"Attempting to send notification with image to ChatID: {chat_id_to_notify} for plate {det_event.plate_number}")
                            ...
                            if success:
                                logging.info(f"Notification with image sent for {det_event.plate_number} to {chat_id_to_notify}.")
                            else:
                                logging.warning(f"Failed to send notification with image for {det_event.plate_number} to {chat_id_to_notify}.")
                        else:
...
if __name__ == '__main__':
    try:
        ...
        if config.has_section('camera_directions'):
            for ip, direction_cfg_val in config.items('camera_directions'): # Renamed to avoid conflict
        ...
        is_placeholder_token = "YOUR_TELEGRAM_BOT_TOKEN_HERE" in bot_token or \
                               ...
                               len(bot_token) < 20 # Arbitrary short length check
        ...
        telegram_notifier = TelegramNotifier(bot_token) # Initialize with the actual token

        # Pass app_config (the main ConfigParser object) to CameraHandler
